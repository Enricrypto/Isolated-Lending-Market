      ...
      <div className='px-8 py-6 border-b border-midnight-700/50 flex items-center justify-between bg-white/5'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-1'>
            {vaults.length} active market{vaults.length !== 1 ? "s" : ""} on
            Sepolia
          </p>
        </div>
        <Link
        ...
        <table className='w-full text-left text-sm'>
          <thead>
            <tr className='border-b border-midnight-700/50 text-[10px] font-bold text-slate-500 uppercase tracking-[0.1em]'>
              ...
              <th className='px-6 py-5'>
                <Tooltip content={TIPS.tvl} side='bottom'>
                  TVL
                </Tooltip>
              </th>
              <th className='px-6 py-5'>
                <Tooltip content={TIPS.supplyApy} side='bottom' width='w-72'>
                  Supply APY
                </Tooltip>
              </th>
              <th className='px-6 py-5'>
                <Tooltip content={TIPS.utilization} side='bottom' width='w-72'>
                  Utilization
                </Tooltip>
              </th>
          ...
          <tbody className='divide-y divide-midnight-700/50'>
            {vaults.map((vault) => {
              ...
              const healthStatus = severityToHealthStatus(
                vault.overallSeverity,
                hasData
              )
              const tvl = formatTVL(
                vault.totalSupply,
              ...
              const supplyAPY = hasData
                ? formatRate(computeSupplyAPY(vault.utilization))
                : "--"
              const borrowAPR = hasData
                ? formatRate(computeBorrowAPR(vault.utilization))
                : "--"
              const utilPct = hasData
