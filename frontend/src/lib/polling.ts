...
// In-memory cache of last snapshot to avoid unnecessary DB reads
let lastSnapshot: Awaited<ReturnType<typeof getLatestSnapshot>> | null = null;
...
export async function pollAndStore() {
  ...
  try {
    ...
    // Use cached snapshot to avoid unnecessary DB read
    const previousSnapshot = lastSnapshot ?? (await getLatestSnapshot());
