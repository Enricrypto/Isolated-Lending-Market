...
contract Market is ReentrancyGuard {
    ...
    address public immutable badDebtAddress;

    /// @notice Vault contract for liquidity management
    ...
    /// @notice User collateral balances (user => token => normalized amount)
    mapping(address => mapping(address => uint256)) public userCollateralBalances;

    /// @notice User's list of deposited collateral assets
    mapping(address => address[]) public userCollateralAssets;

    /// @notice Token decimals for normalization
    mapping(address => uint8) public tokenDecimals;

    /// @notice User's principal debt (normalized to 18 decimals)
    mapping(address => uint256) public userTotalDebt;

    /// @notice User's last borrow index snapshot
    mapping(address => uint256) public lastUpdatedIndex;

    /// @notice Unrecovered debt per user (for statistics/penalties)
    ...
    constructor(
        ...
        loanAsset.approve(_vaultContract, type(uint256).max);
    }
